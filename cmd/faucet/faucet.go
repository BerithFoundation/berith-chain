...
import (
	...
	"github.com/BerithFoundation/berith-chain/internals/version"
...
func main() {
	...
	for i := 0; i < *tiersFlag; i++ {
		...
		amounts[i] = fmt.Sprintf("%s BERS", strconv.FormatFloat(amount, 'f', -1, 64))
		if amount == 1 {
	...
	faucet, err := newFaucet(genesis, *ethPortFlag, *netFlag, ks, website.Bytes())
	if err != nil {
...
func newFaucet(genesis *core.Genesis, port int, network uint64, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// cfg := berith.DefaultTestnetConfig
	// cfg.Genesis = genesis

	// utils.RegisterBerithService(stack, &cfg)
...
func (f *faucet) apiHandler(w http.ResponseWriter, r *http.Request) {
	...
	for {
		...
		if timeout = f.timeouts[id]; time.Now().After(timeout) {
			...
			tx := types.NewTransaction(f.nonce+uint64(len(f.reqs)), address, amount, 21000, f.price, nil, types.Main, types.Main, false)
			...
			if err := f.client.SendTransactionHTTP(signed); err != nil {
				f.lock.Unlock()
				if err = sendError(wsconn, err); err != nil {
...
func (f *faucet) refresh(head *types.Header) error {
	...
	var err error
	if head == nil {
		if head, err = f.client.GetLastHeader(); err != nil {
			return err
		}
	}
	...
	if balance, err = f.client.GetAccountBalance(f.account.Address, head.Number); err != nil {
		return err
	}
	if nonce, err = f.client.GetAccountNonce(f.account.Address, head.Number); err != nil {
		return err
	}
	if price, err = f.client.GetGasPrice(); err != nil {
		return err
	}
...
func (f *faucet) loop() {
	...
	go func() {
		for {
			...
			<-t.C
			head, err := f.client.GetLastHeader()
			if err != nil {
				break
			}
