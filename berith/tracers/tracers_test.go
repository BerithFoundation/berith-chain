...
var makeTest = function(tx, rewind) {
  ...
  var block   = berith.getBlock(berith.getTransaction(tx).blockHash);
  var genesis = berith.getBlock(block.parentHash);
  ...
  genesis.config = admin.nodeInfo.protocols.berith.config;
  ...
  console.log(JSON.stringify({
    ...
    input:  berith.getRawTransaction(tx),
    result: result,
  }, null, 2));
}
*/
...
func TestCallTracer(t *testing.T) {
	...
	for _, file := range files {
		...
		t.Run(camel(strings.TrimSuffix(strings.TrimPrefix(file.Name(), "call_tracer_"), ".json")), func(t *testing.T) {
			...
			if _, _, _, err = st.TransitionDb(types.Main, types.Main); err != nil {
