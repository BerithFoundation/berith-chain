...
func (w *wizard) makeGenesis() {
	...
	fmt.Println(" 3. Bsrr - proof-of-stake")
...
func (w *wizard) manageGenesis() {
	...
	case "2":
		...
		berithJson := filepath.Join(folder, fmt.Sprintf("%s.json", w.network))
		if err := ioutil.WriteFile((berithJson), out, 0644); err != nil {
			log.Error("Failed to save genesis file", "err", err)
			return
		}
		log.Info("Saved native genesis chain spec", "path", berithJson)
