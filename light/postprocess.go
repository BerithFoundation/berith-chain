...
func GetChtRoot(db berithdb.Database, sectionIdx uint64, sectionHead common.Hash) common.Hash {
...
// Note that sectionIdx is specified according to LES/1 CHT section size.
func StoreChtRoot(db berithdb.Database, sectionIdx uint64, sectionHead, root common.Hash) {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
type ChtIndexerBackend struct {
	diskdb, trieTable    berithdb.Database
	odr                  OdrBackend
...
func NewChtIndexer(db berithdb.Database, odr OdrBackend, size, confirms uint64) *core.ChainIndexer {
	trieTable := berithdb.NewTable(db, ChtTablePrefix)
	backend := &ChtIndexerBackend{
		diskdb:      db,
		odr:         odr,
		trieTable:   trieTable,
		triedb:      trie.NewDatabaseWithConfig(trieTable, 1), // Use a tiny cache only to keep memory down
		sectionSize: size,
	}
	return core.NewChainIndexer(db, berithdb.NewTable(db, "chtIndex-"), backend, size, confirms, time.Millisecond*100, "cht")
}
...
func GetBloomTrieRoot(db berithdb.Database, sectionIdx uint64, sectionHead common.Hash) common.Hash {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
func StoreBloomTrieRoot(db berithdb.Database, sectionIdx uint64, sectionHead, root common.Hash) {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
func NewBloomTrieIndexer(db berithdb.Database, odr OdrBackend, parentSize, size uint64) *core.ChainIndexer {
	trieTable := berithdb.NewTable(db, BloomTrieTablePrefix)
	...
	return core.NewChainIndexer(db, berithdb.NewTable(db, "bltIndex-"), backend, size, 0, time.Millisecond*100, "bloomtrie")
}
