...
func gasCallDataCopy(gt params.GasTable, evm *EVM, contract *Contract, stack *Stack, mem *Memory, memorySize uint64) (uint64, error) {
	...
	words, overflow := bigUint64(stack.Back(2).ToBig())
	if overflow {
		return 0, errGasUintOverflow
	}
...
func gasSStore(gt params.GasTable, evm *EVM, contract *Contract, stack *Stack, mem *Memory, memorySize uint64) (uint64, error) {
	var (
		...
		current = evm.StateDB.GetState(contract.Address(), common.BigToHash(x.ToBig()))
	)
	...
	value := common.BigToHash(y.ToBig())
	if current == value { // noop (1)
		return params.NetSstoreNoopGas, nil
	}
	original := evm.StateDB.GetCommittedState(contract.Address(), common.BigToHash(x.ToBig()))
	if original == current {
...
func gasSStoreEIP2200(gt params.GasTable, evm *EVM, contract *Contract, stack *Stack, mem *Memory, memorySize uint64) (uint64, error) {
...
func makeGasLog(n uint64) gasFunc {
	return func(gt params.GasTable, evm *EVM, contract *Contract, stack *Stack, mem *Memory, memorySize uint64) (uint64, error) {
		requestedSize, overflow := bigUint64(stack.Back(1).ToBig())
		if overflow {
			return 0, errGasUintOverflow
		}
...
func gasSha3(gt params.GasTable, evm *EVM, contract *Contract, stack *Stack, mem *Memory, memorySize uint64) (uint64, error) {
	...
	wordGas, overflow := bigUint64(stack.Back(1).ToBig())
	if overflow {
		return 0, errGasUintOverflow
	}
...
func gasCodeCopy(gt params.GasTable, evm *EVM, contract *Contract, stack *Stack, mem *Memory, memorySize uint64) (uint64, error) {
	...
	wordGas, overflow := bigUint64(stack.Back(2).ToBig())
	if overflow {
		return 0, errGasUintOverflow
	}
...
func gasExtCodeCopy(gt params.GasTable, evm *EVM, contract *Contract, stack *Stack, mem *Memory, memorySize uint64) (uint64, error) {
	...
	wordGas, overflow := bigUint64(stack.Back(3).ToBig())
	if overflow {
		return 0, errGasUintOverflow
	}
...
func gasCall(gt params.GasTable, evm *EVM, contract *Contract, stack *Stack, mem *Memory, memorySize uint64) (uint64, error) {
	var (
		...
		address        = common.BigToAddress(stack.Back(1).ToBig())
...
func gasSuicide(gt params.GasTable, evm *EVM, contract *Contract, stack *Stack, mem *Memory, memorySize uint64) (uint64, error) {
	...
	if evm.ChainConfig().IsEIP150(evm.BlockNumber) {
		gas = gt.Suicide
		var (
			address = common.BigToAddress(stack.Back(0).ToBig())
			eip158  = evm.ChainConfig().IsEIP158(evm.BlockNumber)
		)
...
func gasSelfdestruct(gt params.GasTable, evm *EVM, contract *Contract, stack *Stack, mem *Memory, memorySize uint64) (uint64, error) {
	...
	if evm.chainRules.IsEIP150 {
		...
		// [Berith]
		// common.Address(stack.Back(0).Bytes20()) => common.BytesToAddress(stack.Back(0).Bytes())
		var address = common.BytesToAddress(stack.Back(0).Bytes())
