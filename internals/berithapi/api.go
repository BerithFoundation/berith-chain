...
// ProtocolVersion returns the current Berith protocol version this node supports
...
// Sign calculates an Berith ECDSA signature for:
...
// Note, this function is compatible with berith_sign and personal_sign. As such it recovers
// the address of:
// hash = keccak256("\x19Berith Signed Message:\n"${message length}${message})
// addr = ecrecover(hash, signature)
//
// Note, the signature must conform to the secp256k1 curve R, S and V values, where
// the V value must be 27 or 28 for legacy reasons.
//
// https://github.com/BerithFoundation/berith-chain/wiki/Management-APIs#personal_ecRecover
...
// PublicBlockChainAPI provides an API to access the Berith blockchain.
// It offers only methods that operate on public data that is freely available to anyone.
...
func (s *PublicBlockChainAPI) doCall(ctx context.Context, args CallArgs, blockNr rpc.BlockNumber, timeout time.Duration) ([]byte, uint64, bool, error) {
	...
	return result.ReturnData, result.UsedGas, result.Failed(), err
}
...
func newRPCTransaction(tx *types.Transaction, blockHash common.Hash, blockNumber uint64, index uint64, base types.JobWallet, target types.JobWallet) *RPCTransaction {
...
func (args *SendTxArgs) toTransaction() *types.Transaction {
	...
	base := types.ConvertJobWallet(args.Base)
	target := types.ConvertJobWallet(args.Target)

	if args.To == nil {
		return types.NewContractCreation(uint64(*args.Nonce), (*big.Int)(args.Value), uint64(*args.Gas), (*big.Int)(args.GasPrice), input, base, target, false)
	}
	return types.NewTransaction(uint64(*args.Nonce), *args.To, (*big.Int)(args.Value), uint64(*args.Gas), (*big.Int)(args.GasPrice), input, base, target, false)
}
...
// PublicDebugAPI is the collection of Berith APIs exposed over the public
// debugging endpoint.
...
// PrivateDebugAPI is the collection of Berith APIs exposed over the private
// debugging endpoint.
...
// Version returns the current berith protocol version.
