...
function toBerValue(s) {
  ...
  return (
    result.substr(0, result.length - BERITH_UNIT) +
    "." +
    result.substr(result.length - BERITH_UNIT, DISPLAY_UNIT)
  );
}

function getRealtimeBalance(method, address, ...elems) {
  var send = () => {
    sendMessage2("callApi", method, [address, "latest"])
      .then((result) => {
