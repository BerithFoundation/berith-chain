...
// Package berithstats implements the network stats reporting service.
package berithstats
...
func (s *Service) login(conn *websocket.Conn) error {
	...
	if info := infos.Protocols["berith"]; info != nil {
		network = fmt.Sprintf("%d", info.(*berith.NodeInfo).Network)
		protocol = fmt.Sprintf("berith/%d", berith.ProtocolVersions[0])
	} else {
...
func (s *Service) reportLatency(conn *websocket.Conn) error {
	// Send the current time to the berithstats server
	start := time.Now()
	...
	log.Trace("Sending measured latency to berithstats", "latency", latency)
...
func (s *Service) reportBlock(conn *websocket.Conn, block *types.Block) error {
	...
	log.Trace("Sending new block to berithstats", "number", details.Number, "hash", details.Hash)
...
func (s *Service) reportHistory(conn *websocket.Conn, list []uint64) error {
	...
	if len(history) > 0 {
		log.Trace("Sending historical blocks to berithstats", "first", history[0].Number, "last", history[len(history)-1].Number)
	} else {
...
func (s *Service) reportPending(conn *websocket.Conn) error {
	...
	log.Trace("Sending pending transactions to berithstats", "count", pending)
...
func (s *Service) reportStats(conn *websocket.Conn) error {
	...
	log.Trace("Sending node details to berithstats")
