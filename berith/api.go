...
func (api *PrivateDebugAPI) GetBadBlocks(ctx context.Context) ([]*BadBlockArgs, error) {
	...
	for i, block := range blocks {
		...
		if results[i].Block, err = berithapi.RPCMarshalBlock(block, true, true); err != nil {
...
func (s *PublicEthAPI) GetBlockByNumber(ctx context.Context, blockNr rpc.BlockNumber, fullTx bool) (map[string]interface{}, error) {

	block, err := s.e.APIBackend.BlockByNumber(ctx, blockNr)
...
func (s *PublicEthAPI) GetBalance(ctx context.Context, address common.Address, blockNr rpc.BlockNumber) (*hexutil.Big, error) {
	state, _, err := s.e.APIBackend.StateAndHeaderByNumber(ctx, blockNr)
...
// a `BlockchainAPI`.
func (s *PublicEthAPI) rpcMarshalBlock(ctx context.Context, b *types.Block, inclTx bool, fullTx bool) (map[string]interface{}, error) {
	fields, err := RPCMarshalBlock(b, inclTx, fullTx, s.e.APIBackend.ChainConfig())
	if err != nil {
		return nil, err
	}
	if inclTx {
		fields["totalDifficulty"] = (*hexutil.Big)(s.e.APIBackend.GetTd(b.Hash()))
	}
	return fields, err
}
