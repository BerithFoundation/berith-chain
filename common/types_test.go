...
func TestMarshalUnmarshalText(t *testing.T) {
	invalidPrefix := "0x"
	if strings.EqualFold(AddressPrefix, "0x") {
		invalidPrefix = "AB"
	}
	var tests = []struct {
		Input     string
		...
		{"", true, nil},
		{`""`, true, nil},
		{`"` + invalidPrefix + `"`, true, nil},
		{AddressPrefix + "0000000000000000000000000000000000000000", false, big.NewInt(0)},
		{AddressPrefix + "0000000000000000000000000000000000000010", false, big.NewInt(16)},
	}
	for i, test := range tests {
		var v Address
		err := v.UnmarshalText([]byte(test.Input))
...
func TestAddressUnmarshalJSON(t *testing.T) {
	// If not use hex prefix, then "0x" is invalid prefix,
	// otherwise "AB" is invalid prefix
	invalidPrefix := "0x"
	if strings.EqualFold(AddressPrefix, "0x") {
		invalidPrefix = "AB"
	}
	var tests = []struct {
		...
		{"", true, nil},
		{`""`, true, nil},
		{`"` + AddressPrefix + `"`, true, nil},
		{`"` + AddressPrefix + `00"`, true, nil},
		{`"` + invalidPrefix + `0000000000000000000000000000000000000010"`, true, nil},
		{`"` + AddressPrefix + `G000000000000000000000000000000000000000"`, true, nil},
		{`"` + AddressPrefix + `0000000000000000000000000000000000000000"`, false, big.NewInt(0)},
		{`"` + AddressPrefix + `0000000000000000000000000000000000000010"`, false, big.NewInt(16)},
	}
	for i, test := range tests {
...
func TestAddressHexChecksum(t *testing.T) {
	var tests = []struct {
		...
		{"0xfb6916095ca1df60bb79ce92ce3ea74c37c5d359", "fB6916095ca1df60bB79Ce92cE3Ea74c37c5d359"},
		{"0xdbf03b407c01e7cd3cbea99509d93f8dddc8c6fb", "dbF03B407c01E7cD3CBea99509d93f8DDDC8C6FB"},
		{"0xd1220a0cf47c7b9be7a2e6ba89f429762e7b9adb", "D1220A0cf47c7B9Be7A2E6BA89F429762e7b9aDb"},
		// Ensure that non-standard length input values are handled correctly
		{"0xa", "000000000000000000000000000000000000000A"},
		{"0x0a", "000000000000000000000000000000000000000A"},
		{"0x00a", "000000000000000000000000000000000000000A"},
		{"0x000000000000000000000000000000000000000a", "000000000000000000000000000000000000000A"},
	}
	for i, test := range tests {
		output := HexToAddress(test.Input).Hex()
		test.Output = AddressPrefix + test.Output
...
func TestMixedcaseAccount_Address(t *testing.T) {
	...
	if err := json.Unmarshal([]byte(`[
		{"A" : "Bxae967917c465db8578ca9024c205720b1a3651A9", "Valid": false},
		{"A" : "BxAe967917c465db8578ca9024c205720b1a3651A9", "Valid": true},
		{"A" : "Bx1111111111111111111112222222222223333323", "Valid": true}
		]`), &res); err != nil {
		t.Fatal(err)
	}
	...
	for i, r := range res {
		if got := r.A.ValidChecksum(); got != r.Valid {
			t.Errorf("test #%d: expected checksum %v, got checksum %v, input %v", i, r.Valid, got, r.A.String())
		}
	}
	...
	for _, r := range []string{
		`["` + AddressPrefix + `11111111111111111111122222222222233333"]`,     // Too short
		`["` + AddressPrefix + `111111111111111111111222222222222333332"]`,    // Too short
		`["` + AddressPrefix + `11111111111111111111122222222222233333234"]`,  // Too long
		`["` + AddressPrefix + `111111111111111111111222222222222333332344"]`, // Too long
		`["1111111111111111111112222222222223333323"]`,                        // Missing address prefix
		`["x1111111111111111111112222222222223333323"]`,                       // Missing address prefix
		`["` + AddressPrefix + `G111111111111111111112222222222223333323"]`,   //Non-hex
	} {
