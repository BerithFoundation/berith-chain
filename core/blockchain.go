...
// Package core implements the Berith consensus protocol.
package core
...
type BlockChain struct {
	...
	db     berithdb.Database // Low level persistent database to store final content in
...
// block is retrieved from the blockchain's internals cache.
func (bc *BlockChain) CurrentBlock() *types.Block {
	return bc.currentBlock.Load().(*types.Block)
}

// CurrentFastBlock retrieves the current fast-sync head block of the canonical
// chain. The block is retrieved from the blockchain's internals cache.
...
func (bc *BlockChain) insert(block *types.Block) {
	...
	// 임시로 커밋했던 빈 블록이 먼저 등록되어있을 경우 업데이트 해야함
...
func (bc *BlockChain) InsertReceiptChain(blockChain types.Blocks, receiptChain []types.Receipts) (int, error) {
	...
	for i, block := range blockChain {
		...
		if batch.ValueSize() >= berithdb.IdealBatchSize {
...
// event about them
//
// rorgs는 오래된 체인과 새로운 체인의 두 블록을 가지고 블록들을 재구성한 뒤
// 새로운 표준 체인의 일부로 삽입한다. 이후 잠재적인 누락된 트랜잭션을 모아서
// 그에 대한 이벤트를 전송한다.
func (bc *BlockChain) reorg(oldBlock, newBlock *types.Block) error {
	...
	if oldBlock.NumberU64() > newBlock.NumberU64() {
		// reduce old chain
		// 추가될 체인이 더 짧다면 oldBlock의 체인을 newBlock의 체인의 높이까지
		// 거슬러 올라가며 oldChain에 추가
	...
	// 높이가 같은 newBlock과 oldBlock에서 공통 부모 블록을 찾아가는 과정
	for {
		if oldBlock.Hash() == newBlock.Hash() {
			commonBlock = oldBlock // 공통 부모 블록
			break
		}
	...
	if len(oldChain) > 0 && len(newChain) > 0 {
		logFn := log.Debug
		if len(oldChain) > 63 { // 체인이 꽤 오래전부터 갈라져왔기 때문에 로그를 경고 단계로 격상
			logFn = log.Warn
		}
	...
	for i := len(newChain) - 1; i >= 0; i-- { // 공통부모 블록까지 newChain으로 싹 갈아치움
		// insert the block in the canonical way, re-writing history

		// 추후 WriteBlockwithState에서 한번 더 인자로 넘어온 newBlock을 insert하지만
		// newChain[len(newChain) - 1]과 같은 블록이기에 크게 상관없음
		//
	...
	// 트랜잭션이 DB에서 삭제되었다면 포크에 생성된 영수증도 삭제해야 한다.
	batch := bc.db.NewBatch()
	for _, tx := range diff {
