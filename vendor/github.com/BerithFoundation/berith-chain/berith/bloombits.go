...
const (
	// bloomServiceThreads is the number of goroutines used globally by an Berith
...
func (e *Berith) startBloomHandlers(sectionSize uint64) {
...
// for the Berith header bloom filters, permitting blazing fast filtering.
type BloomIndexer struct {
	size    uint64               // section size to generate bloombits for
	db      berithdb.Database    // database instance to write index data and metadata into
...
func NewBloomIndexer(db berithdb.Database, size, confirms uint64) *core.ChainIndexer {
	...
	table := berithdb.NewTable(db, string(rawdb.BloomBitsIndexPrefix))
