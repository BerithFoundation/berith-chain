...
func TestEmptySync(t *testing.T) {
	dbA := NewDatabase(berithdb.NewMemDatabase())
	dbB := NewDatabase(berithdb.NewMemDatabase())
	emptyA, _ := New(common.Hash{}, dbA)
	emptyB, _ := New(emptyRoot, dbB)

	for i, trie := range []*Trie{emptyA, emptyB} {
		if req := NewSync(trie.Hash(), berithdb.NewMemDatabase(), nil).Missing(1); len(req) != 0 {
