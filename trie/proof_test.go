...
// test all variations.
func makeProvers(trie *Trie) []func(key []byte) *berithdb.MemDatabase {
	var provers []func(key []byte) *berithdb.MemDatabase

	// Create a direct trie based Merkle prover
	provers = append(provers, func(key []byte) *berithdb.MemDatabase {
		proof := berithdb.NewMemDatabase()
...
func BenchmarkProve(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		proofs := berithdb.NewMemDatabase()
...
func BenchmarkVerifyProof(b *testing.B) {
	...
	root := trie.Hash()
	var keys []string
	var proofs []*berithdb.MemDatabase
	for k := range vals {
		keys = append(keys, k)
		proof := berithdb.NewMemDatabase()
