...
func (w *wizard) deployDashboard() {
	...
	for _, service := range []string{"berithstats", "explorer", "wallet", "faucet"} {
	...
	// If we have berithstats running, ask whether to make the secret public or not
	if w.conf.ethstats != "" {
		fmt.Println()
		fmt.Println("Include berithstats secret on dashboard (y/n)? (default = yes)")
