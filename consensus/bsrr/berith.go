	...
	for _, tx := range txs {
		...
		if msg.Target() == types.Stake {
			...
			if state != nil {
				...
				stake_block := new(big.Int).Set(state.GetStakeUpdated(msg.From()))
...
선출확율 반환 함수
*/
func (c *BSRR) getJoinRatio(stakingList *staking.Stakers, address common.Address, blockNumber uint64, states *state.StateDB) (float64, error) {
	return 0, nil
}
